...
struct tcpsock {
  ...
  time_t timestamp;  // keep track of the lats timestamp of the socket 
  int first_time;    //keep track of if the socket is the first time comming in 
  } ;		
...
int tcp_passive_open(tcpsock_t ** sock, int port)
{
  ...
  tcpsock_t * s = tcp_sock_create();     //create a new socket
  ...
  addr.sin_port = htons(port);               //The htons() function converts the unsigned short integer hostshort from host byte order to network byte order.  it means that in memory the number 5001 will be stored as 0x89 0x13
  result = bind(s->sd,(struct sockaddr *)&addr,sizeof(addr));//When a socket is created with socket(2) it exists in a name space(address family) but has no address assigned to it.  bind() assigns the address specified by addr to the socket referred to by the file descriptor sockfd.
  TCP_DEBUG_PRINTF(result==-1,"Bind() failed with errno = %d [%s]", errno, strerror(errno));
  TCP_ERR_HANDLER(result!=0,free(s);return TCP_SOCKOP_ERROR);   // if success return 0;
...
int tcp_active_open(tcpsock_t ** sock, int remote_port, char * remote_ip)
{
  ...
  memset(&addr, 0, sizeof(struct sockaddr_in));
  addr.sin_family = PROTOCOLFAMILY;
  result = inet_aton(remote_ip, (struct in_addr *) &addr.sifirst_timen_addr.s_addr);//converts the Internet host address cp from the IPv4 numbers-and-dots notation into binary form (in network byte order) and stores it in the structure that inp points to
...
int  tcp_get_tp(tcpsock_t * socket,time_t **tp)
{
    TCP_ERR_HANDLER(socket==NULL,return TCP_SOCKET_ERROR);
    TCP_ERR_HANDLER(socket->cookie!=MAGIC_COOKIE,return TCP_SOCKET_ERROR); 
    *tp=&(socket->timestamp);
...
int  tcp_get_first_time(tcpsock_t * socket,int **ft)
{
    TCP_ERR_HANDLER(socket==NULL,return TCP_SOCKET_ERROR);
    TCP_ERR_HANDLER(socket->cookie!=MAGIC_COOKIE,return TCP_SOCKET_ERROR); 
    *ft=&(socket->first_time);
    return TCP_NO_ERROR;
}