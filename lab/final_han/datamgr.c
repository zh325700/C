 ...
 void datamgr_parse_sensor_data(FILE * fp_sensor_map, sbuffer_t ** buffer)
{
    ...
    sensor_element->last_modified=0;
...
void * element_copy_mgr(void * element) {
    ...
    copy ->flag =((sensor_element_t*)element)->flag;
    copy->index =((sensor_element_t*)element)->index;
    while(i>=0)
    {
        copy->last_sensor_data[i]=((sensor_element_t*)element)->last_sensor_data[i];
        i--;
    }
    return (void *) copy;
}
