...
int tcp_active_open(tcpsock_t ** sock, int remote_port, char * remote_ip)
{
  ...
  memset(&addr, 0, sizeof(struct sockaddr_in));
  addr.sin_family = PROTOCOLFAMILY;
  result = inet_aton(remote_ip, (struct in_addr *) &addr.sin_addr.s_addr);//converts the Internet host address cp from the IPv4 numbers-and-dots notation into binary form (in network byte order) and stores it in the structure that inp points to
