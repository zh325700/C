...
err_code_t stack_save( const stack_t * stack, const char *path  )
{
  ...
  // write stack->numElement to file
  result = write( fd, &(stack->num_elements), sizeof(stack->num_elements) );
  if ( result == -1 ) {
    perror("File write failed: ");
    close(fd);
    return ERR_FILE;
  }
  if ( result != sizeof(stack->num_elements ) ) {
  ...
  while ( t != NULL )
  {
    ...
    result = write( fd, &(t->element), sizeof(element_t) );
    if ( result == -1 ) {
      perror("File write failed: ");
      close(fd);
      return ERR_FILE;
    }
    if ( result != sizeof(element_t) ) {
      fprintf(stderr, "File write failed: less bytes written than requested: \n");
      close(fd);
      return ERR_FILE;
    }
...
err_code_t stack_restore( stack_t * stack, const char *path  )
{
  ...
  result = read( fd, &count_elements, sizeof(stack->num_elements) );
  if ( result == -1 ) {
  ...
  if ( result != sizeof(stack->num_elements ) ) {
    fprintf(stderr, "File read failed: less bytes read (%d) than requested (%d): \n", result, (int)sizeof(stack->num_elements) );
    close(fd);
    return ERR_FILE;
  }
  ...
  // double check size of stack
  if ( count_elements != (result - sizeof(stack->num_elements)) / sizeof(element_t) ) {
  ...
  for ( offset = sizeof(element_t); count_elements; offset += sizeof(element_t), count_elements-- )
  {
    ...
    result = read( fd, &element, sizeof(element_t) );
    if ( result == -1 ) {
      perror("File read failed: ");
      close(fd);
      return ERR_FILE;
